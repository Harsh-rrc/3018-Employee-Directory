...
describe('Branch routes - update & delete', () => {
  it('should update branch (200)', async () => {
    const res = await request(app).put('/api/v1/branches/1').send({ phone: '1-999-888-7777' });
    expect(res.status).toBe(200);
    expect(res.body.data.phone).toBe('1-999-888-7777');
  });

  it('should return 404 updating nonexistent branch', async () => {
    const res = await request(app).put('/api/v1/branches/9999').send({ phone: '999-888-7777' });
    expect(res.status).toBe(400);
  });
